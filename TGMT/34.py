...
def pipeline_model(img_path):
    ...
    faceDetectionModel = os.path.join(path, 'model/res10_300x300_ssd_iter_140000_fp16.caffemodel')
 
    # Face detection model architecture
    faceDetectionProto = os.path.join(path, 'model/deploy.prototxt.txt')
 
    # Face descriptor model
    faceDescriptor = os.path.join(path, 'model/openface.nn4.small2.v1.t7')
...
image_path = os.path.join(path, 'dataimage_40.jpg')
